...
#iptables -A INPUT -i $INT_LAN -s $CLIENT -d $GATEWAY_LAN -p tcp --dport 22 -m mac --mac-source  -j ACCEPT
...
iptables -A FWDDMZ  -i $INT_WAN -o $INT_DMZ -d $SRV_DMZ -p tcp -m multiport --dports 443,80,22 -j ACCEPT
iptables -A FWDDMZ  -i $INT_DMZ -o $INT_WAN -s $SRV_DMZ -p tcp -m multiport --sports 443,80,22 -j ACCEPT
	#SSH DMZ->ROUTER
